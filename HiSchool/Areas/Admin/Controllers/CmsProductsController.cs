...
namespace HiSchool.Areas.Admin.Controllers
{
    public class CmsProductsController : Controller
    {
        ...
        private readonly CmsProductsAction _sysAction = new CmsProductsAction();
        ...
        {
            var objTemporary = new CmsProducts();
            try
            {
        ...
        [HttpPost]
        public ActionResult Edit(CmsProducts sysModel)
        {
            try
            {
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<CmsProducts>().Updates(sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<CmsProducts>().Deletes(sWhere);
