...
namespace HiSchool.Areas.Admin.Controllers
{
    public class CmsServiceController : Controller
    {
        ...
        private readonly CmsServiceAction _sysAction = new CmsServiceAction();
        ...
        public ActionResult GetData(int? page, int? pageSize, string key, int? sort, string usedState)
        {
            try
            {
                ...
                objRef[1] = sWhere;
                objRef[2] = sSort;
                objRef[3] = currentPageIndex;
                objRef[4] = pageSize;
                objRef[5] = 0;
                var modelList = _sysAction.List(ref objRef);
                int totalRow = CommonHelper.Convert.ConvertToInt32(objRef[5]);
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<CmsService>().Updates( sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<CmsService>().Deletes( sWhere);
