...
{
    public class Ex_ExamConfigController : Controller
    {
        #region Properties Class

        private readonly Ex_ExamConfigAction _sysAction = new Ex_ExamConfigAction();
        ...
        public ActionResult GetData(int? page, int? pageSize, string key, int? sort, string usedState)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(key))
                {
                    key = key.Trim();
                    ViewBag.Search = key;
                    sWhere += "and  ( a.ExamCode like N'%" + key + "%' or dbo.ConvertToNoSigned(a.ExamCode) like  dbo.ConvertToNoSigned(N'%" + key + "%')) ";
                }
        ...
        public ActionResult GetChuong(string key, int id)
        {
            try
            {
                ...
                string sSort = "a.Orders DESC";
                var modelList = new SysAdminAction<Ex_ExamConfig>().List(sWhere, sSort, 0, 100);
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<Ex_ExamConfig>().Updates(sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<Ex_ExamConfig>().Deletes(sWhere);
