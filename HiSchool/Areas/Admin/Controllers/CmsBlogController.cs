...
{
    public class CmsBlogController : Controller
    {
        #region Properties Class

        private readonly CmsBlogAction _sysAction = new CmsBlogAction();
        ...
        [HttpPost]
        public ActionResult Edit(CmsBlog sysModel)
        {
            try
            {
                ...
                if (sysModel.Id > 0)
                {
                    ...
                    if (AuthorizeUser.IsAction(Ultity.Constant.Edit) || AuthorizeUser.IsAction(Ultity.Constant.ViewEditAll))
                    {
                        ...
                        sysModel.NumberView = objTemporary.NumberView;
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<CmsBlog>().Updates( sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<CmsBlog>().Deletes( sWhere);
