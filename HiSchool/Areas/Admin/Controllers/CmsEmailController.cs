...
namespace HiSchool.Areas.Admin.Controllers
{
    public class CmsEmailController : Controller
    {
        ...
        private readonly CmsEmailAction _sysAction = new CmsEmailAction();
        ...
        public ActionResult GetData(int? page, int? pageSize, string key, int? sort, string usedState)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(key))
                {
                    key = key.Trim();
                    ViewBag.Search = key;
                    sWhere += "and  ( a.Email like N'%" + key + "%' or dbo.ConvertToNoSigned(a.Email) like  dbo.ConvertToNoSigned(N'%" + key + "%')) ";
                }
        ...
        [HttpPost]
        public ActionResult Edit(CmsEmail sysModel)
        {
            try
            {
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<CmsEmail>().Updates(sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<CmsEmail>().Deletes( sWhere);
