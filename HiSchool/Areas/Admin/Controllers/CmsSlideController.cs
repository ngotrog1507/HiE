...
{
    public class CmsSlideController : Controller
    {
        #region Properties Class

        private readonly CmsSlideAction _sysAction = new CmsSlideAction();
        ...
        public ActionResult GetData(int? page, int? pageSize, string key, int? sort, string usedState)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(key))
                {
                    key = key.Trim();
                    ViewBag.Search = key;
                    sWhere += "and  ( a.Title like N'%" + key + "%' or dbo.ConvertToNoSigned(a.Title) like  dbo.ConvertToNoSigned(N'%" + key + "%')) ";
                }
        ...
        [HttpPost]
        public ActionResult Edit(CmsSlide sysModel)
        {
            try
            {
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<CmsSlide>().Updates( sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<CmsSlide>().Deletes( sWhere);
