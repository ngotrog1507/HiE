...
namespace HiSchool.Areas.Admin.Controllers
{
    ...
    public class CmsCategoryController : Controller
    {
        ...
        private readonly CmsCategoryAction _sysAction = new CmsCategoryAction();
            ...
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    if (Request.IsAjaxRequest())
                    {
                        ...
                        string tableName = "CmsCategory";
                        if (!AuthorizeUser.IsHost())
                        {
                            new SysAdminAction<CmsCategory>().Updates(sSet, sWhere);
                            return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                        }
                        else
                        {
                            new SysAdminAction<CmsCategory>().Deletes( sWhere);
