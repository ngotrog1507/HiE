...
{
    public class Ex_SubjectController : Controller
    {
        #region Properties Class

        private readonly Ex_SubjectAction _sysAction = new Ex_SubjectAction();
        ...
        public ActionResult GetChuong(string key, int id)
        {
            try
            {
                ...
                var modelList = new SysAdminAction<Ex_Subject>().List(sWhere, sSort, 0, 100);
        ...
        {
            var objTemporary = new Ex_Subject();
            try
            {
                int editId = id.HasValue ? id.Value : 0;
                if (editId > 0)
                {
                    objTemporary = new SysAdminAction<Ex_Subject>().GetById(editId);
                }
        ...
        [HttpPost]
        public ActionResult Edit(Ex_Subject sysModel)
        {
            try
            {
                ...
                if (sysModel.Id > 0)
                {
                    var objTemporary = new SysAdminAction<Ex_Subject>().GetById(sysModel.Id);
        ...
        public ActionResult Delete(string id)
        {
            try
            {
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    {
                        new SysAdminAction<Ex_Subject>().Updates(sSet, sWhere);
                        return Json(new { status = "success", Message = Ultity.Constant.Change_Delete_Success });
                    }
                    else
                    {
                        new SysAdminAction<Ex_Subject>().Deletes(sWhere);
